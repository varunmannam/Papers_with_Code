...
class VAE(nn.Module):
    def __init__(self, image_channels=3, h_dim=3072, z_dim=10000):
...
vae = torch.load('VAE_var_test2_10000.pt').to(device)
#print (vae)
#print ('here>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>',vae)
...
class Regression(nn.Module):
    def __init__(self, D_in, D_out, p=0.0):
        ...
        self.p = p
        self.linear1 = th.nn.Linear(D_in, 6000) #1000 to 6000 similar to 9000 one
        self.f1 = th.nn.ReLU()
        self.linear2 = th.nn.Linear(6000, 4000)
        self.f2 = th.nn.ReLU()
        self.linear2a = th.nn.Linear(4000, 2000)
        self.f2a = th.nn.ReLU()
        self.linear2b = th.nn.Linear(2000, 500)
        self.f2b = th.nn.ReLU()
        self.linear2c = th.nn.Linear(500, 100)
        self.f2c = th.nn.ReLU()
        self.linear3 = th.nn.Linear(100, D_out)
...
model = Regression(D_in = 10000, D_out = 10).to(device)
